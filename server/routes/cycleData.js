...
async function requireFeatureEnabled(req, res, next) {
  try {
    const result = await pool.query("SELECT value FROM config WHERE key = 'feature_cycle_data'");
    const enabled = result.rows.length === 0 || String(result.rows[0].value).toLowerCase() !== 'false';
    if (!enabled) {
...
router.get('/admin-list', requireAdmin, async (req, res) => {
  ...
  try {
    let query = 'SELECT id, username, start_date, end_date, filename, created_at FROM cycle_data_uploads';
    const params = [];
    const conditions = [];
...
router.get('/download-zip', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    const query = `
      SELECT username, start_date, end_date, filename 
      FROM cycle_data_uploads
