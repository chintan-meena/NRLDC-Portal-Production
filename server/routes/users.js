...
router.post('/:username/assignments', requireSelfOrAdmin('username'), requireSameRegion(), async (req, res) => {
  ...
  try {
    ...
    // The one-pending-per-plant index tripped: two submissions raced past the
    // app guard. Report it the same way the guard does rather than as a 500.
    if (err.code === '23505') {
      return res.status(409).json({ error: `"${acronym}" already has a transfer request awaiting RLDC Admin approval.` });
    }
...
router.post('/transfer-requests', async (req, res) => {
  ...
  try {
    ...
    // The one-pending-per-plant index tripped: a concurrent submission won the
    // race. Same 409 the app guard returns.
    if (err.code === '23505') {
      return res.status(409).json({ error: `"${acronym}" already has a transfer request awaiting RLDC Admin approval.` });
    }
